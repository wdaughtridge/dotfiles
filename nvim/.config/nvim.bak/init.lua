...
vim.api.nvim_create_autocmd("BufEnter", {
	...
	callback = function()
		local root_dir =
			vim.fs.dirname(vim.fs.find({ "*.xcodeproj", ".git", "ContentView.swift" }, { upward = true })[1])
...
vim.api.nvim_create_autocmd("BufEnter", {
	pattern = "*.go",
	callback = function()
		local root_dir = vim.fs.dirname(vim.fs.find({ "main.go", "go.mod", "go.sum", ".git" }, { upward = true })[1])
		local client = vim.lsp.start({
			name = "gopls",
			cmd = { "gopls", "serve" },
			root_dir = root_dir,
...
vim.api.nvim_create_autocmd("BufEnter", {
	pattern = "*.tf",
	callback = function()
		local root_dir = vim.fs.dirname(vim.fs.find({ "main.tf", ".git" }, { upward = true })[1])
		local client = vim.lsp.start({
			name = "terraform-ls",
